% Book information
\titlehead{}
\title{L'identification des projets de logiciel libre accessibles aux nouveaux contributeurs}
\subtitle{Mémoire de M2 Sciences de l'Éducation, parcours SYNVA}
\author[PH]{Paul HERVOT}
\date{\today}
\publishers{
    Jury :

    \bigskip

    \begin{tabular}{lr}
        Benoît CRESPIN & directeur\\
        Richard NGU LEUBOU & assesseur
    \end{tabular}

    \bigskip

    \begin{figure}[ht]
        \includegraphics[width=0.5\textwidth]{unistra}
    \end{figure}
}
\dedication{%
    Un grand merci à Antoine Pietri pour son aide précieuse lors de la collecte des données sur le graphe de
    Software Heritage.

    \emph{Deux} grands mercis à Marc Trestini et Benoît Crespin, ayant par la force des choses tous deux
    endossé le rôle de directeur de mémoire à différentes étapes de son écriture, leurs expertises
    complémentaires m'ayant permis d'arriver au bout de ce travail.%
}

% Pre-document content
\setchapterstyle{plain}
\pagelayout{wide}

\input{document_start}

% Main document content
\pagestyle{scrheadings}
\setchapterstyle{kao}

\pagelayout{margin}
\include{introduction}
\pagelayout{wide}

\addpart{Partie théorique}

\pagelayout{margin}
\include{literature_review}
\pagelayout{wide}

\addpart{Partie expérimentale}

\include{methodology}

\include{results}

\addpart{Conclusion}

\include{conclusion}

% Number chapters with letters from now on
\appendix

\addpart{Annexes}

\include{annexes}

% End of the main document content
\backmatter

\include{document_end}

