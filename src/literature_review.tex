...
volontaires quitter le projet après leur première contribution \sideparencite{hadoop-dropout-2013}. Les études
...
\sideparencite{gender-information-processing-1995,gender-information-processing-2015}. De façon peut être plus
...
fonctionnalités \sideparencite{gender-programming-2010,gender-programming-2006}. Une des tendances trouvées
par \textcite[p.~1008]{barriers-2018} est que la majorité ($58\%$) des barrières rencontrées dans leur étude
sont de nature sociale ("\en{community-oriented}") et non techniques. Concernant les aspects plus techniques,
...
et de réduction des barrières d'entrée pour ces population en minorité permettraient probablement d'augmenter
la valeur globale de ce type de projets.
...
\begin{itemize}
    \item le manque d'interactions sociales avec les membres du projet
        \sideparencite{barriers-meta-2015,social-barriers-2015} ;
...
\todo[inline]{Voir si ces articles sont pertinents à citer}
...
par exemple de grandement faciliter la détection des "clones" ou "\en{forks}" de projets, c'est à dire des
projets ayant une part de leur historique de développement en commun. Un exemple célèbre de ce phénomène est
LibreOffice\sidenote{\url{https://www.libreoffice.org/}}, un logiciel libre de bureautique ayant démarré en se
basant sur le code source d'OpenOffice\sidenote{\url{https://openoffice.apache.org/}} et étant maintenant un
projet autonome avec une équipe de développement différente. La structure de donnée proposée par
\textcite{swh-graph-2020} est un graphe orienté où chaque \engl{commit} est représenté par un nœuds et où
...
prédecesseur de $v$"), ce qui signifie que trouver tous les "\en{forks}" d'un projet se fait avec un classique
calcul de composante fortement connexe à partir de n'importe lequel de ses \englpl{commit}.
...
recherche \sideparencite{swh-2017}. Il s'agit du corpus que \textcite{swh-graph-2020} ont utilisé comme
exemple pour leur modèle de compression. L'archive comptait en 2018 plus d'un milliard de \englpl{commit}
uniques archivés depuis quatre-vingt-cinq millions d'origines différentes (telles que des \en{remotes}
\engl{git} ou des versions de paquets Python publiés sur PyPi\sidenote{\url{https://pypi.org/}}) et augmente
de façon continue en ajoutant de nouveaux projets et en suivant l'évolution des projets déjà archivés
\sidetextcite{swh-2019}.

\todo[inline]{%
    TODO : expliquer ici la structure du graphe ? Ou plutôt dans la partie méthodologie ? Remarque j'explique
    déjà le point le plus critique en parlant de \textcite{swh-graph-2020}.
}
...
L'entretien lui-même se fait en suivant un processus appelé "\en{GenderMag}", lui-même un type de
"\en{Cognitive Walkthrough}" qui consiste à présenter aux sujets interrogés un personnage fictif avec ses
...
\sideparencite{gendermag-2016,cognitive-walkthrough-2000}
...
Quand \sidetextcite{mining-github-2014} analysent les \englpl{pull request} sur \gls{github}, ils ne
retiennent que les projets ayant au moins deux auteurs de \englpl{commit} uniques, ce qu'ils considèrent comme
